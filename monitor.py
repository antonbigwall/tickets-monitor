...
def check_tickets():
    try:
        ...
        # Находим блок про входные билеты
        idx = text.find("входные билеты")
        if idx == -1:
            print("Блок 'входные билеты' не найден на странице")
            return False

        # Берём текст вокруг этого блока (500 символов до и 300 после)
        chunk = text[max(0, idx - 500): idx + 300]

        # Если рядом нет "распроданы" — билеты появились!
        if "распроданы" not in chunk:
            return True
